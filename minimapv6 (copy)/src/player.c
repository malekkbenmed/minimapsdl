...
void update_player(Player* player) {
    ...
    int playerLeftX = player->position.x;
    int playerRightX = player->position.x + player->position.w;
    int maxGroundY = -1;

    for (int x = playerLeftX; x <= playerRightX; x++) {
        if (x >= 0 && x < SCREEN_WIDTH && groundHeight[x] > maxGroundY) {
            maxGroundY = groundHeight[x];
        }
    }

    if (maxGroundY != -1 && (player->position.y + player->position.h) > maxGroundY) {
        player->position.y = maxGroundY - player->position.h;
        player->velocityY = 0;
        player->jumping = 0;
    }
