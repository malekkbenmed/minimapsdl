...
SDL_Rect minimap_rect = {10, 10, 150, 100};
...
void init_minimap() {
    ...
    SDL_FreeSurface(temp);
    if(!platform_icon) {
        fprintf(stderr, "Failed to convert platform icon\n");
        cleanup_game();
        exit(1);
    }
    ...
    SDL_FreeSurface(temp);
    if(!coin_icon) {
        fprintf(stderr, "Failed to convert coin icon\n");
        cleanup_game();
        exit(1);
    }
}
...
void draw_minimap() {
    ...
    // Draw the player icon using scaled coordinates.
    SDL_Rect p_pos = {
    ...
    // Draw the platform icon if active.
