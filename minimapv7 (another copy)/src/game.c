...
void init_game() {
    ...
    memset(game.coins_collected, 0, sizeof(game.coins_collected));
    load_level(0);
...
void save_game() {
    ...
    for(int i=0; i<MAX_LEVELS; i++) {
        fprintf(f, "%d ", game.coins_collected[i]);
    }
    fclose(f);
...
void load_game() {
    ...
    fscanf(f,"%d %d %d %d %d %hd",
           ...
           &current_level);
    for(int i=0; i<MAX_LEVELS; i++) {
        fscanf(f, "%d ", &game.coins_collected[i]);
    }
    fclose(f);
