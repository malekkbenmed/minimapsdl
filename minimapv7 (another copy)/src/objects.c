...
void init_objects() {
    ...
    switch(current_level) {
        ...
        case 2:
            platform.active = 0;
            obstacle.active = 0;
            subwayEntry = (SDL_Rect){545, 450, 100, 50};
            coin.position.x = 300;
            break;
        case 3:
            ...
            break;
        case 4:
            platform.position.x = 250;
            coin.position.x = 550;
            obstacle.position.x = 350;
            break;
        case 5:
            platform.position.x = 600;
            coin.position.x = 100;
            obstacle.position.x = 300;
            break;
    }

    coin.active = !game.coins_collected[current_level];
...
void update_objects() {
    if (coin.active && check_collision(player.position, coin.position)) {
        coin.active = 0;
        game.coins_collected[current_level] = 1;
        game.score += 1;
        flashBackground = 1;
...
void draw_objects() {
    ...
    if (current_level == 2 && check_collision(player.position, subwayEntry)) {
        render_text(game.screen, font, "PRESS E TO ENTER SUBWAY", subwayEntry.x - 50, subwayEntry.y - 30);
    }
