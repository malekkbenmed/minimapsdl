...
static void update_src_rect(Player* player, int row, int frame_index) {
    ...
    // Ensure at least 1 frame to prevent division by zero
...
void update_player(Player* player) {
 ...
 // Level transitions and subway interaction
    const Uint8* keys = SDL_GetKeyState(NULL);
    if (current_level == 2) { // City 3
        // Block right edge
        ...
        // Subway interaction
        if (check_collision(player->position, subwayEntry)) {
            if (keys[SDLK_e]) {
                load_level(3); // Jump to City 4
                player->position.x = 0;
            }
        }
    } else {
        // Normal level transitions for other levels
        if (player->position.x + player->position.w > SCREEN_WIDTH) {
...
// Replace horizontal clamping with screen transition checks
    if(player->position.x + player->position.w > SCREEN_WIDTH) {
        load_level(current_level + 1);
        player->position.x = 0;
    }
