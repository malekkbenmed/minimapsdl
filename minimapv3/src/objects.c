...
void init_objects() {
    // Initialize platform: attach it to the ground.
    ...
    // Set platform so its bottom touches GROUND_LEVEL.
    ...
    // Initialize coin: animated coin with a sprite sheet (4 frames)
    ...
    // Assume each frame is 32x32; you can adjust if needed.
    coin.max_frame = 4;
    coin.frame = 0;
    coin.position.x = 600;
    coin.position.y = GROUND_LEVEL - 200;  // Position as desired.
    coin.position.w = 32;
    coin.position.h = 32;
    coin.active = 1;
}
